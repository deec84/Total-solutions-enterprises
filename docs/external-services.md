...
|---|---|---|---|---|
| GitHub account | Private `deec84/Total-solutions-enterprises` repository, Actions enabled, protected environments, ruleset, Dependabot, private vulnerability reporting | Developer SSH key or GitHub CLI login; GitHub-managed `GITHUB_TOKEN` | Developer keychain / GitHub | All hosted gates and collaboration |
...
| Terraform backend | Per-environment encrypted/versioned S3 state bucket, customer-managed KMS key, native `.tflock`, access logging, public-access blocking, and recovery controls | Backend names; operator/OIDC authorization | Secure bootstrap records, never source-controlled backend files or tfvars | Any safe Terraform apply |
...
| Language/accessibility approval | Spanish product/legal reviewer, VoiceOver/TalkBack device matrix, accessibility owner, localized store copy and screenshots | Approved copy and signed review evidence; no application secret | Controlled document system / store consoles | Spanish production launch and localized store submission |
