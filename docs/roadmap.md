...
## 17. Billing entitlements and store-verification foundation — implemented; store/provider gates required

Provider-neutral product and entitlement contracts, fail-closed server verification, HMAC-pseudonymous transaction references, idempotent current-state and append-only event ledgers, account-deletion unlinking, privacy export, disabled provider/native-store adapters, no-store APIs, Flutter membership/restore flows, and disabled-by-default Terraform configuration. Prices are never invented or supplied by the backend. Exit: unit, contract, API, repository, PostgreSQL retention/unlinking, mobile, migration, security, coverage, and exact-commit hosted gates pass; accepting money remains blocked on approved Apple/Google products and agreements, tax/terms/refund decisions, a real native billing bridge, production verification credentials, sandbox purchase tests, and explicit release authorization.

## 18. Internationalization and accessibility — implemented; hosted/native/language-review gates required

Generated Flutter localization catalogs with complete English/Spanish key parity, locale-aware navigation and feature surfaces, localized trust/provenance labels, date/currency formatting, native Android/iOS metadata and permission-purpose text, stable non-presentational error codes, accessible live regions, adaptable map controls, and 200% text-scale regression coverage. Unrecognized machine values remain visibly labeled, and provider, community, municipal, sign, and AI response content is never silently translated or presented as official. Exit: catalog/native-metadata validation, Flutter Analyze, bilingual widget/unit/accessibility tests, unchanged 75% maintained-source coverage, Android build, unsigned iOS build, and exact-commit hosted gates pass; production language launch remains blocked on owner-approved Spanish legal/product review, physical-device VoiceOver/TalkBack evaluation, and localized store-listing approval.
