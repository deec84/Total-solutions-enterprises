...
The membership UI is connected to ParkShield's server-authoritative entitlement API, but the runtime `StorePurchaseBridge` is intentionally disabled. The app cannot initiate a charge until a reviewed native Apple/Google billing adapter, real store products, server verification/notification paths, sandbox evidence, legal/finance approval, and explicit release authorization exist. Prices and transaction confirmation must come from the device store, never hard-coded Dart or backend values.
