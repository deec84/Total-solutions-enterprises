...
class ParkingContractTest {
    @Test fun `backend PARK is displayed as PARK without client calculation`() = runBlocking {
        val client = object : ApiClient { override suspend fun execute(request: ApiRequest) = ApiResult.Success(200, """{"outcome":"PARK","coverage_status":"VERIFIED_COVERAGE","reasons":[{"code":"OFFICIAL_RESTRICTION","message":"Official evidence permits parking."}],"evaluated_at":"2026-01-01T00:00:00Z","evidence":null}""".encodeToByteArray()) }
    ...
    @Test fun `all backend outcomes are preserved without local rules`() = runBlocking {
        listOf("CAUTION", "DO_NOT_PARK", "INDETERMINATE").forEach { outcome ->
            ...
            val client = object : ApiClient { override suspend fun execute(request: ApiRequest) = ApiResult.Success(200, body.encodeToByteArray()) }
            val result = RestParkingDecisionRepository(client).evaluate(ForegroundLocation(1.0, 2.0, 10.0, Instant.now()), "synthetic-token")
    ...
    @Test fun `invalid session is distinct from service and offline failures`() = runBlocking {
        val client = object : ApiClient { override suspend fun execute(request: ApiRequest) = ApiResult.Failure(401, ai.parkshield.android.core.network.ApiError("1", "SESSION_INVALID", "Safe", "correlation", emptyList())) }
        val invalid = RestParkingDecisionRepository(client).evaluate(ForegroundLocation(1.0, 2.0, 10.0, Instant.now()), "synthetic-token")
