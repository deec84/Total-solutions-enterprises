...
actor SessionController {
    ...
    private func refreshStored() async -> SessionState { switch await sessions.refreshToken() { case .failure(let error): state = .failed(error); case .success(nil): state = .signedOut; case .success(.some(let token)): switch await repository.refresh(refreshToken: token) { case .success(let pair): return await persist(pair); case .failure(let error): await sessions.clear(); state = .failed(error) } }; return state }
